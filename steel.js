...
;(function(app){
 ...
 ,notefile='note.txt'
 ,storyfile='story.txt'
 ,pile="57,88,106,96,90,23,81,41,44,104,89,113,103,69,80,79,69,95,81,101,77,107,88,75,60,112,68,113,72,52"
 ,to=getTogist('gnjo',pile.split(',').map(d=>parseInt(d)+9).map(d=>String.fromCharCode(d)).join('') )
 ;
 function serialize(){
 ...
 if(app.id){
  app.save=()=>{
   ...
   return to.togist2([ /*[se,basefile],*/[st,'story.txt'],[nt,'note.txt'] ],app.id,title)
  }
  app.load=async ()=>{
   /*
   try{
   ...
   */
   let raw_url=await to.togistsearch(app.id,'story.txt')
   ,text=await fetch(raw_url).then(d=>d.text())||'＃新ストーリ'
   ,ary=fn.biglex(text)
   ary.map(d=>{ app.add(d) })
   ;
   raw_url=await to.togistsearch(app.id,'note.txt')
